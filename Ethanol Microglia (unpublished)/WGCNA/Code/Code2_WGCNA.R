...
workingDir = "Z:/Russell/WGCNA - EtOH/DataSheets";
setwd(workingDir); 
...
Data = read.csv("GeneTableIP_DeSeq.csv");
...
# Figure Directory
FigDir = "Z:/Russell/WGCNA - EtOH/Figures";
setwd(FigDir); 

# Plot the results:
sizeGrWindow(9, 5)
cex1 = 0.75;
png("Soft Power Threshold IP_DeSeq.png");
...
adjacency_signed = adjacency(GeneCounts, power= 6, type="signed", corFnc="bicor");
...
sizeGrWindow(12,9)
plot(geneTree, xlab="", sub="", main = "ETOH RNA-Seq IP - DeSeq", labels = FALSE, hang = 0.04);
...
sizeGrWindow(8,6)
plotDendroAndColors(geneTree, moduleColor, "Dynamic Tree Cut", dendroLabels = FALSE, hang = 0.03, addGuide = TRUE, guideHang = 0.05, main = "ETOH Microglia RNA-Seq")
...
sizeGrWindow(18, 9)
png("Merged gene Dendrogram IP DeSeq.png")
...
# Figure Directory
DataDir = "Z:/Russell/WGCNA - EtOH/R";
setwd(DataDir); 

# Save dissTOM
write.csv(dissTOM_signed, file = "dissTOM IP DeSeq.csv",  row.names=FALSE)

# Save Module Eigengenes
write.csv(MEs, file = "MEs IP DeSeq.csv",  row.names=FALSE)

# Save Module Hub Genes
hubs = chooseTopHubInEachModule(GeneCounts, moduleColor)
write.csv(hubs, file = "Hubs IP DeSeq.csv",  row.names=FALSE)

# Save Module Membership & Gene Count
geneInfo = data.frame(names(GeneCounts),moduleColor,t(GeneCounts));
write.csv(geneInfo, file = "Gene Modules IP DeSeq.csv")
...
write.csv(geneInfo, file = "Gene Modules Merged IP DeSeq.csv")

# Save Module Hub Genes
hubs = chooseTopHubInEachModule(GeneCounts, mergedColors)
write.csv(hubs, file = "Hubs Merged IP DeSeq.csv",  row.names=FALSE)

# Save Module Eigengenes Merged
write.csv(mergedMEs, file = "MEs Merged IP DeSeq.csv",  row.names=FALSE)
#=====================================================================================
#
...
# If necessary, change the path below to the directory where the data files are stored. 
workingDir = "Z:/Neumaier Lab/Morphine Grant/RNA Seq 2/WGCNA Final Pipeline/Data Sheets";
setwd(workingDir); 
...
# Figure Directory
FigDir = "Z:/Neumaier Lab/Morphine Grant/RNA Seq 2/WGCNA Final Pipeline/Figures";
setwd(FigDir); 
...
adjacency_signed = adjacency(GeneCounts, power= 16, type="signed", corFnc="bicor");
TOM_signed = TOMsimilarity(adjacency_signed, TOMType="signed", verbose=3);
dissTOM_signed = 1-TOM_signed;
geneTree = hclust(as.dist(dissTOM_signed), method = "average");

sizeGrWindow(12,9)
plot(geneTree, xlab="", sub="", main = "Morphine Microglia RNA-Seq Input", labels = FALSE, hang = 0.04);
dynamicMods_hybrid = cutreeDynamic(dendro = geneTree, distM = dissTOM_signed, method="hybrid", deepSplit = 3, pamRespectsDendro = TRUE, minClusterSize = 50);
moduleColor = labels2colors(dynamicMods_hybrid)
sizeGrWindow(8,6)
...
# Figure Directory
DataDir = "Z:/Neumaier Lab/Morphine Grant/RNA Seq 2/WGCNA Final Pipeline/WGCNA Output";
setwd(DataDir); 
...
# Save Module Eigengenes
write.csv(mergedMEs, file = "MEs IN.csv",  row.names=FALSE)

# Save Module Hub Genes
hubs = chooseTopHubInEachModule(GeneCounts, moduleColor)
write.csv(hubs, file = "Hubs IN.csv",  row.names=FALSE)
